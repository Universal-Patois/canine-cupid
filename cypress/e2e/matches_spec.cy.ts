...
describe('Matched Dog Page spec', () => {
  ...
  it('should be able to see the page header and nav bar links', () => {
    cy.contains('.app-title', "Canine Cupid - A Wag Worthy Match")
    cy.contains('.links', 'Favorites')
  });

  it('should see a section header and swiper carousel of dog matches with the dogs picture, breed name, info icon, and favorite icon', () => {
  ...
  it("should be able to click through the swiper buttons to go through matched dogs", () => {
  ...
  it("should be able to view more dog info page when info icon is clicked", () => {
    cy.get('.info-icon').first().click()
    cy.url().should('include', '/American%20Staffordshire%20Terrier')
  });

  it("should be able to favorite and unfavorite dogs when heart icon is clicked", () => {
